...
async function captureVisibleTab() {
  ...
  try {
    ...
    const vrect0 = viewEl ? viewEl.getBoundingClientRect() : { left: 0, top: 0, width: vw, height: vh };
    const scaleX = vw / window.innerWidth, scaleY = vh / window.innerHeight;
    const cropW = Math.max(1, Math.round(vrect0.width * scaleX));
    const cropH = Math.max(1, Math.round(vrect0.height * scaleY));
    const vrect = vrect0;
    if (cropW <= 0 || cropH <= 0) return null;

    /* 위치(left·top)는 매번 다시 잰다 — 상태 문구가 접히는 줄 수를 바꾸거나
       레이아웃이 미세하게 밀리면 화면 위 좌표가 살짝 움직일 수 있어서,
       한 번만 재고 계속 쓰면 아래쪽 구간에서 옆(아래쪽 상태줄 등)이 딸려
       찍히는 문제가 있었다. 크기(너비·높이)는 안 바뀌니 처음 값을 그대로 쓴다 */
    const grab = () => {
      const r = viewEl.getBoundingClientRect();
      const cropX = Math.max(0, Math.round(r.left * scaleX));
      const cropY = Math.max(0, Math.round(r.top * scaleY));
