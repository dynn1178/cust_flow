...
function webHintHtml(defaultText) {
  return '<span class="weburl-hint" style="color:var(--ink-3); font-size:11.5px">' + defaultText + "</span>";
}
...
function harCaptureScript() {
  return "(function(){" +
    ...
    /* GA4 등 오래된 방식의 트래킹은 fetch/XHR/sendBeacon이 아니라
       new Image().src=... 처럼 이미지 태그로 신호만 보내는 "픽셀 비콘"을
       쓰기도 한다 — 이건 저 셋을 다 가로채도 안 잡혀서 결과가 비어 보이는
       흔한 원인이다. img.src에 값을 넣는 순간도 같이 가로챈다. */
    "var imgDesc=Object.getOwnPropertyDescriptor(HTMLImageElement.prototype,'src');" +
    "if(imgDesc&&imgDesc.set){Object.defineProperty(HTMLImageElement.prototype,'src',{configurable:true,enumerable:imgDesc.enumerable,get:imgDesc.get,set:function(v){try{push(v,'GET',null);}catch(e){}return imgDesc.set.call(this,v);}});}" +
    /* sendBeacon은 문자열 대신 Blob으로 본문을 실어 보내는 경우도 흔한데,
       Blob은 만들어진 뒤엔 내용을 동기적으로 다시 읽을 방법이 없다 — 그래서
       Blob이 "만들어지는 순간"(원본 조각이 아직 문자열 그대로일 때) 미리
       내용을 기억해 뒀다가, sendBeacon에 그 Blob이 오면 그때 꺼내 쓴다. */
    "var oBlob=window.Blob,blobText=new WeakMap();" +
    "window.Blob=function(parts,opts){var b=new oBlob(parts,opts);try{blobText.set(b,(parts||[]).map(function(p){return typeof p==='string'?p:'';}).join(''));}catch(e){}return b;};" +
    "window.Blob.prototype=oBlob.prototype;" +
    "var oB=navigator.sendBeacon?navigator.sendBeacon.bind(navigator):null;" +
    "if(oB){navigator.sendBeacon=function(u,d){var body=null;try{body=typeof d==='string'?d:(d&&blobText.has(d)?blobText.get(d):null);}catch(e){}push(u,'POST',body);return oB(u,d);};}" +
    "window.__tagcapRestore=[function(){window.fetch=oF;},function(){X.prototype.open=oO;X.prototype.send=oS;}," +
      "function(){if(imgDesc)Object.defineProperty(HTMLImageElement.prototype,'src',imgDesc);}," +
      "function(){window.Blob=oBlob;}," +
      "function(){if(oB)navigator.sendBeacon=oB;}];" +
