...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    try {
      /* "load"(모든 하위 자원까지 다 끝나기를 기다림) 대신 "domcontentloaded"만
         기다린다 — 광고·추적 스크립트가 계속 떠 있는 페이지는 "load"가 몇 초씩
         안 끝나기도 해서 캡처 시간의 대부분을 이게 잡아먹었다. 동영상·오디오는
         화면을 찍는 데 필요 없으니 아예 안 받아서 그만큼도 줄인다. */
      await page.setRequestInterception(true);
      page.on("request", r => { if (r.resourceType() === "media") r.abort(); else r.continue(); });
      try {
        await page.goto(target.href, { waitUntil: "domcontentloaded", timeout: 8000 });
        await new Promise(r => setTimeout(r, 500));   // 위 내용이 막 그려질 잠깐의 여유
