...
| 역할 | 여정 지도·태그 열람 | 캠페인·성과 추이 열람 | 편집·삭제 | CSV·JSON 다운로드 | 저장 | 회원 권한 지정 |
| --- | --- | --- | --- | --- | --- | --- |
| **서버관리자** | O | O | O | O | O | O (양도 포함) |
| **운영자** | O | O | O | O | O | X |
| **일반회원** | O | X | X | X | X | X |
| **비회원(로그인 안 함)** | X | X | X | X | X | X |

- 비회원은 RLS(`jta_docs_read` 등)가 서버에서 막아 열람 자체가 안 되고, 홈 화면에 구글 로그인을 안내하는 창이 뜹니다. (Supabase Anonymous Sign-Ins 기능은 쓰지 않습니다 — 같은 프로젝트를 다른 서비스와 공유할 경우 그쪽에도 익명 접속이 열리는 등 영향 범위가 넓어 보류했습니다.)
- 캠페인·성과 추이는 구글 시트 원본이라 `/api/sheets`가 별도로 지키며, **운영자 이상만** 볼 수 있습니다(일반회원은 탭 자체가 안 보이고, 서버에서도 401/403으로 막힙니다).
